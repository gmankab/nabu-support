...
Version:        6.14
Release:        7%{?dist}
Summary:        Fedora‑style Linux kernel, modules and headers for Qualcomm SM8150
License:        GPLv2
URL:            https://gitlab.com/sm8150-mainline/linux
Source0:        %{url}/-/archive/sm8150/%{version}/linux-sm8150-%{version}.tar.gz
...
%prep
%autosetup  -n linux-sm8150-%{version}
...
ln -sr %{buildroot}/usr/lib/modules/%{uname_r}/vmlinuz \
       %{buildroot}/boot/vmlinuz-%{uname_r}

install -d  %{buildroot}/usr/lib/ostree-boot
touch %{buildroot}/usr/lib/modules/%{uname_r}/initramfs.img
ln -s  ../modules/%{uname_r}/vmlinuz         \
       %{buildroot}/usr/lib/ostree-boot/vmlinuz-%{uname_r}
ln -s  ../modules/%{uname_r}/initramfs.img   \
       %{buildroot}/usr/lib/ostree-boot/initramfs-%{uname_r}.img
